1) Install kubernetes on docker desktop(windows 11):
    ...
    11) Run in CMD: `docker compose -f k8s/docker-compose-kafka-k8s.yml up --build --force-recreate`
    ...
    18) execute run_route.sh or do manual 19,20,21(after pod rebuild need rerun script)
    19) route port from pod to local network:
        `kubectl port-forward pods/sb-kafka-hibernate-pod 8083:8083 --namespace=sb-kafka`
    20) route port from pod to local network:
        `kubectl port-forward pods/sb-kafka-cons-pod 8082:8082 --namespace=sb-kafka`
    21) route port from pod to local network:
